...
UKL_ELF="${UKL_ELF:-vmlinux}"
OUTDIR="${OUTDIR:-../../results/ukl/wx}"
mkdir -p "$OUTDIR"
...
{
  echo "[*] UKL W^X check"
  echo "[*] Target ELF: $UKL_ELF"
  echo "[*] Note: this checks vmlinux/uncompressed ELF, not bzImage."

  if [ ! -f "$UKL_ELF" ]; then
    echo "RESULT: FAIL (UKL ELF not found)"
    exit 1
  fi

  if ! file "$UKL_ELF" | grep -q "ELF"; then
    echo "RESULT: FAIL (target is not an ELF file)"
    exit 1
  fi

  load_wx_count=$(
    readelf -lW "$UKL_ELF" | awk '
      /LOAD/ {
        if ($0 ~ /W/ && $0 ~ /E/) c++
      }
      END { print c+0 }
